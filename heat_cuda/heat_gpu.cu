...
int main(void)
{
    ...
    float* u1 = (float*)calloc(N_POINTS, sizeof(float));
    ...
    for (int n = 0; n < MAX_ITER; n++)
    {
        ...
        cudaMemcpy(d_u1, u1, N_POINTS*sizeof(float), cudaMemcpyHostToDevice);

        heat_kernel<<<numBlocks, threadsPerBlock>>>(d_u0, d_u1);

        cudaMemcpy(u0, d_u0, N_POINTS*sizeof(float), cudaMemcpyDeviceToHost);
        cudaMemcpy(u1, d_u1, N_POINTS*sizeof(float), cudaMemcpyDeviceToHost);
